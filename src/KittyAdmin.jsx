          ...
          {schemes.map((s) => (
            <tr key={s.id} style={{ borderBottom: "1px solid #eee" }}>
              ...
              <td><button onClick={() => setEditing({
                ...
                perks: { ...emptyScheme().perks, ...(s.perks || {}) }, description: s.description || "", active: s.active, sortOrder: s.sort_order,
              })}>Edit</button></td>
            </tr>
          ))}
        </tbody>
...
function SchemeEditor({ scheme, funnels, crmSecret, onDone, onCancel }) {
  ...
  const save = async () => {
    ...
    const cleanPerks = Object.fromEntries(
      Object.entries(f.perks).filter(([, v]) => v !== "" && v !== false).map(([k, v]) => [k, typeof v === "boolean" ? v : (isNaN(Number(v)) ? v : Number(v))])
    );
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {enrollments.map((e) => (
            <div key={e.id} style={{ border: "1px solid #ddd", borderRadius: 8, padding: 12 }}>
              ...
              {e.status === "completed" && e.claim_status !== "claimed" && (
                <button onClick={() => updateClaim(e.id, "claimed")} style={{ marginLeft: 8 }}>Mark Claimed</button>
              )}
...
function LegacyTab({ crmSecret }) {
  ...
  const submit = async () => {
    ...
    setSaving(true);
    const d = await call("add-legacy-member", { method: "POST", crmSecret, body: f });
    setSaving(false);
    if (d.ok) { setMsg("Added — claim reminders will start automatically."); setF({ name: "", phone: "", legacySchemeName: "", notes: "" }); }
