...
function EnrollmentsTab({ crmSecret, actor, onNewEnroll, lockedSchemeSlug }) {
  ...
  const setMonthlyRate = async (schemeId, monthStr) => {
    ...
    if (!confirm(
      ...
      `Every affected member gets a WhatsApp with their rate + gold added. Anyone who hasn't paid this month yet also gets a reminder to pay today to lock this rate.`
    )) return;
    const d = await call("set-monthly-rate", { method: "POST", crmSecret, body: { schemeId, month: monthStr, ratePerGram: rate, actor } });
    if (d.ok) { alert(`Rate applied to ${d.updated} payment${d.updated === 1 ? "" : "s"} · ${d.notified || 0} member${d.notified === 1 ? "" : "s"} notified · ${d.reminded || 0} unpaid member${d.reminded === 1 ? "" : "s"} reminded on WhatsApp.`); load(); } else alert(d.error);
  };
