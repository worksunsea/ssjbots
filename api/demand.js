...
export default async function handler(req, res) {
  ...
  try {
    ...
    let lead;
    if (body.leadId) {
      const { data } = await sb.from("bullion_leads").select("*").eq("id", body.leadId).single();
      lead = data;
    }
    if (!lead) {
      // Upsert by phone
    ...
    // 6. Send opening message via WhatsApp
    const wa = await sendWhatsApp({ phone, msg: openingMsg });
    ...
    if (visitScheduledAt) {
      ...
      const clientName = lead.name || body.name || "Sir/Ma'am";
    ...
    return res.status(200).json({ ok: true, demandId: demand.id, leadId: lead.id, sent, openingMsg });
  } catch (err) {
