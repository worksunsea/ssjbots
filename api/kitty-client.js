...
export default async function handler(req, res) {
  ...
  if (action === "list") {
    ...
    const { data, error } = await sb.from("kitty_enrollments")
      .select("id,status,is_legacy,legacy_scheme_name,start_date,claim_status,claimed_at,created_at,member_number,razorpay_subscription_id,frozen_at,swarn_frequency,monthly_amount_override,cycle_number,scheme:kitty_schemes(name,monthly_amount,duration_months,perks),installments:kitty_installments(month_number,due_date,amount,status,paid_at,paid_amount,rate_locked,grams_purchased,source),redemptions:kitty_redemptions(redemption_type,item_description,value,redeemed_at)")
